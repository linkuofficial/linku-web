...
- check.mjs 新增兩節（沿用「任一 FAIL → exit 1」慣例，門檻數字實作時可微調但必須存在）：
  - **4. 資源預算**：assets/render.js ≤ 48KB、assets/main.js ≤ 16KB、assets/styles.css ≤ 44KB、assets/proof.js ≤ 14KB（未壓縮；現況 main.js 約 12KB、styles 約 22KB；render.js 為 P1 新檔、proof.js 為 P-proof 新檔——13.6KB 含控制理論註解文件，2026-07-08 實作時定為 14KB）。
